...
const CodeHeader: FC<CodeHeaderProps> = ({ language, code }) => {
  ...
  return (
    <div className="aui-code-header-root border-border/50 bg-muted/50 mt-2.5 flex items-center justify-between border-2 border-b-0 px-3 py-1.5 text-xs">
...
const defaultComponents = memoizeMarkdownComponents({
  ...
  th: ({ className, ...props }) => (
    <th
      className={cn(
        "aui-md-th bg-muted px-2 py-1 text-start font-medium [[align=center]]:text-center [[align=right]]:text-right",
        className,
      )}
      {...props}
    />
  ...
  tr: ({ className, ...props }) => (
    <tr
      className={cn(
        "aui-md-tr m-0 border-b p-0 first:border-t",
        className,
      )}
      {...props}
    />
  ...
  pre: ({ className, ...props }) => (
    <pre
      className={cn(
        "aui-md-pre border-border/50 bg-muted/30 overflow-x-auto border-2 border-t-0 p-3 text-xs leading-relaxed",
        className,
      )}
      {...props}
    />
  ...
  code: function Code({ className, ...props }) {
    ...
    return (
      <code
        className={cn(
          !isCodeBlock &&
            "aui-md-inline-code border-border/50 bg-muted/50 border px-1.5 py-0.5 font-mono text-[0.85em]",
          className,
        )}
        {...props}
      />
